...
Estado: v0.2, actualizado con la fundación local B2-010 de medios. Se seguirá ampliando cuando existan endpoints, herramientas y proveedores desplegados.
...
| ------ | -------------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------- |
| TM-001 | Cliente se hace pasar por Fer                      | Cambios de precio/stock o exfiltración   | identidad inmutable, vínculo explícito contact→member, membresía vigente, snapshot por actor y autorización por herramienta | texto de suplantación sigue como contact; miembro suspendido no obtiene run ni tools |
...
| TM-009 | SSRF por medios/URLs                               | acceso a red interna/metadata            | origen Supabase exacto, URL firmada same-origin, sin URLs arbitrarias, timeout y egress | origen externo, credenciales, path/query base y protocolo inseguro se bloquean |
| TM-010 | Archivo malicioso/bomba                            | RCE/DoS/costo                            | límites por bucket y descarga streaming acotada; MIME real/transcodificación aislada en B3-005 | vacío, MIME distinto, tamaño declarado o stream excedido fallan cerrados |
...
| TM-022 | Borrado lógico mal diseñado                        | pérdida de historial                     | estados, soft-delete, triggers append-only y restricciones referenciales | producto o evidencia de medios vendidos no se elimina físicamente |
...
| TM-025 | Medio privado o URL durable se filtra               | exposición de catálogo interno/PII y acceso cross-tenant | original, análisis y WhatsApp privados; sólo WebP aprobado público; rutas/hash en DB, nunca bytes/Base64/URL firmada; RLS y TTL 30–900 s | otro tenant ve cero rutas; auditoría no guarda URLs; publicación previa a aprobación falla |
