...
import { b4008DatabaseMutants } from "./database-b4-008-mutants.mjs";
...
assert.ok(npmExecutable, "B4-008 mutation testing must be invoked through npm");
...
const reportFile = path.join(reportDirectory, "linked-b4-008-mutation-summary.json");
const outcomes = [];
await mkdir(temporaryDirectory, { recursive: true });

for (const [index, mutant] of b4008DatabaseMutants.entries()) {
  ...
  const temporaryFile = path.join(
    temporaryDirectory,
    `linked-b4-008-mutation-${process.pid}-${index}.sql`,
  );
...
assert.ok(
  ...
  "B4-008 pgTAP must kill every Facebook Business Login database mutant",
);
console.log(
  `Linked B4-008 database mutation gate verified: ${outcomes.length}/${outcomes.length} killed (100%) and rolled back on ${expectedProjectName}.`,
);
