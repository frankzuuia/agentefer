...
import { b2010DatabaseMutants } from "./database-b2-010-mutants.mjs";

const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const repositoryRoot = path.resolve(scriptDirectory, "..");
const expectedProjectRef = "hprdctmblmfcoagugvyp";
const expectedProjectName = "AgenteFer";
const migrationRelativePath = "supabase/migrations/20260828190000_b2_010_media_storage.sql";
const testRelativePath = "supabase/tests/b2_010_media_storage_test.sql";
const npmExecutable = process.env.npm_execpath;
const maxBuffer = 50 * 1024 * 1024;

assert.ok(npmExecutable, "B2-010 mutation testing must be invoked through npm");
...
const reportFile = path.join(reportDirectory, "linked-b2-010-mutation-summary.json");
const outcomes = [];
...
for (const [index, mutant] of b2010DatabaseMutants.entries()) {
  ...
  const temporaryFile = path.join(
    temporaryDirectory,
    `linked-b2-010-mutation-${process.pid}-${index}.sql`,
  );
...
assert.equal(
  outcomes.length,
  b2010DatabaseMutants.length,
  "every B2-010 database mutant must execute",
);
assert.ok(
  outcomes.every((outcome) => outcome.killed),
  "B2-010 pgTAP must kill every media storage database mutant",
);

console.log(
  `Linked B2-010 database mutation gate verified: ${outcomes.length}/${outcomes.length} killed (100%) and rolled back on ${expectedProjectName}.`,
);
