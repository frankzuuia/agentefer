...
import { b3001aPreparationDatabaseMutants } from "./database-b3-001a-preparation-mutants.mjs";
...
const migrationRelativePath =
  "supabase/migrations/20260828123000_b3_001a_tool_preparation_scaling.sql";
const testRelativePath = "supabase/tests/b3_001a_tool_preparation_scaling_test.sql";
const npmExecutable = process.env.npm_execpath;
const maxBuffer = 50 * 1024 * 1024;

assert.ok(npmExecutable, "B3-001A preparation mutation testing must be invoked through npm");
...
const [migrationSource, testSource] = await Promise.all([
  readFile(path.join(repositoryRoot, migrationRelativePath), "utf8"),
  readFile(path.join(repositoryRoot, testRelativePath), "utf8"),
]);
...
const reportFile = path.join(reportDirectory, "linked-b3-001a-preparation-mutation-summary.json");
const outcomes = [];
...
for (const [index, mutant] of b3001aPreparationDatabaseMutants.entries()) {
  ...
  const temporaryFile = path.join(
    temporaryDirectory,
    `linked-b3-001a-preparation-mutation-${process.pid}-${index}.sql`,
  );
...
assert.equal(
  outcomes.length,
  b3001aPreparationDatabaseMutants.length,
  "every B3-001A preparation database mutant must execute",
);
assert.ok(
  outcomes.every((outcome) => outcome.killed),
  "B3-001A preparation pgTAP must kill every database mutant",
);

console.log(
  `Linked B3-001A preparation mutation gate verified: ${outcomes.length}/${outcomes.length} killed (100%) and rolled back on ${expectedProjectName}.`,
);
