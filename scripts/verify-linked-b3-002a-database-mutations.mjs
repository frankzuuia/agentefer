...
import { b3002aDatabaseMutants } from "./database-b3-002a-mutants.mjs";
...
const migrationRelativePath =
  "supabase/migrations/20260828160000_b3_002a_whatsapp_actor_resolution.sql";
const testRelativePath = "supabase/tests/b3_002a_whatsapp_actor_resolution_test.sql";
const npmExecutable = process.env.npm_execpath;
const maxBuffer = 50 * 1024 * 1024;

assert.ok(npmExecutable, "B3-002A mutation testing must be invoked through npm");
...
const reportFile = path.join(reportDirectory, "linked-b3-002a-mutation-summary.json");
const outcomes = [];
...
for (const [index, mutant] of b3002aDatabaseMutants.entries()) {
  ...
  const temporaryFile = path.join(
    temporaryDirectory,
    `linked-b3-002a-mutation-${process.pid}-${index}.sql`,
  );
...
assert.equal(
  outcomes.length,
  b3002aDatabaseMutants.length,
  "every B3-002A database mutant must execute",
);
assert.ok(
  outcomes.every((outcome) => outcome.killed),
  "B3-002A pgTAP must kill every pending database mutant",
);

console.log(
  `Linked B3-002A database mutation gate verified: ${outcomes.length}/${outcomes.length} killed (100%) and rolled back on ${expectedProjectName}.`,
);
