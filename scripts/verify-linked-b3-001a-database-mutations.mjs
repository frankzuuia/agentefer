...
import { b3001aDatabaseMutants } from "./database-b3-001a-mutants.mjs";
...
const migrationRelativePath =
  "supabase/migrations/20260827170000_b3_001a_read_only_agent_tools.sql";
const testRelativePath = "supabase/tests/b3_001a_read_only_agent_tools_test.sql";
const npmExecutable = process.env.npm_execpath;
const maxBuffer = 50 * 1024 * 1024;

assert.ok(npmExecutable, "B3-001A mutation testing must be invoked through npm");
...
const reportFile = path.join(reportDirectory, "linked-b3-001a-mutation-summary.json");
const outcomes = [];
...
for (const [index, mutant] of b3001aDatabaseMutants.entries()) {
  ...
  const temporaryFile = path.join(
    temporaryDirectory,
    `linked-b3-001a-mutation-${process.pid}-${index}.sql`,
  );
...
assert.equal(
  outcomes.length,
  b3001aDatabaseMutants.length,
  "every B3-001A database mutant must execute",
);
assert.ok(
  outcomes.every((outcome) => outcome.killed),
  "B3-001A pgTAP must kill every pending database mutant",
);

console.log(
  `Linked B3-001A database mutation gate verified: ${outcomes.length}/${outcomes.length} killed (100%) and rolled back on ${expectedProjectName}.`,
);
