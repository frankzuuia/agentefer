...
-- The wrapper is the only API claim surface. Move its implementation delegate
-- out of api so PostgREST cannot expose a second, context-bypassing endpoint.
alter function api.claim_whatsapp_agent_turn_authorized_base(
  text, text, text, text, text, text, integer, uuid
) set schema app_private;

revoke all on function app_private.claim_whatsapp_agent_turn_authorized_base(
  text, text, text, text, text, text, integer, uuid
